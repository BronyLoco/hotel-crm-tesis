...
// Actualizar estado de una habitación (Usado por otros microservicios)
const updateRoomStatus = async (req, res) => {
  try {
    const { number } = req.params; // Ej: "101"
    const { status } = req.body;   // Ej: "OCCUPIED"
    ...
    res.json({ message: `Estado de habitación ${number} actualizado a ${status}`, room });
  } catch (error) {
    res.status(500).json({ error: error.message });
  }
};
